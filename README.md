...
```bash
load-shedding-cal-app-rs$ sam build
```
...
```bash
load-shedding-cal-app-rs$ sam local invoke LoadShedCalFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
load-shedding-cal-app-rs$ sam local start-api
load-shedding-cal-app-rs$ curl http://localhost:3000/
```
...
```bash
load-shedding-cal-app-rs$ sam logs -n LoadShedCalFunction --stack-name load-shedding-cal-app-rs --tail
```
